...
def standardize_html(file_path, lang_code="pt-br"):
    ...
    # SEO
    hreflangs = get_hreflang_tags(filename)
...
for lang, folder in languages.items():
    if os.path.exists(folder):
        for f in os.listdir(folder):
            if f.endswith(".html"):
                standardize_html(os.path.join(folder, f), lang)

# Calculadora específica
calc_path = "ferramentas/salary-split-calculator/index.html"
if os.path.exists(calc_path):
    standardize_html(calc_path, "en") # A calculadora original está em EN
